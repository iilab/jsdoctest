...
exports.run = function jsdoctest$run(filename) {
  ...
  console.log('jsdoctest$run.filename', filename);
...
exports.getJsdoctests = function jsdoctest$getJsdoctests(content) {
  ...
  var comments = _.map(functionComments, function(comment) {
    ...
    var examples = _.flatten(
      _.map(exampleNodes, function(exampleNode) {
        return commentParser.run(exampleNode.string);
      })
    );

    var overrideTags = _.filter(comment.tags, { type: 'function' });
    var override = _.flatten(
      _.map(overrideTags, function(overrideTag) {
        return overrideTag.string;
      })
    );
    comment.ctx = comment.ctx ? comment.ctx : { string: override.join(' ') };
...
exports.toJsdocRegister = function jsdoctest$toJsdocRegister(comment) {
  ...
  var compiled = _.map(comment.examples, function(example) {
    var id = escapeString(baseId) + escapeString(example.testCase) + ' => ' + escapeString(example.expectedResult);
...
if (path.basename(process.argv[1]) === '_mocha' && !JSDOCTEST_DISABLE) {
  ...
  if (module.parent.exports === mocha) {
    console.log('mocha --require support delete');
