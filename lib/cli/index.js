...
exports.run = function(argv) {
  program
    ...
    .option('-s, --spec', 'Outputs generated `mocha` specs to stdout rather than running tests')
  ...
  for (var i = 0, len = program.args.length; i < len; i++) {
    var filename = path.resolve(process.cwd(), program.args[i]);
    var failed = jsdoctest.run(filename);
...
exports._init = function init() {
  ...
  else json.scripts.test = '';
  json.scripts.test += 'npm run jsdoctest';
  json.scripts.jsdoctest = 'mocha --require jsdoctest `find lib src source -name "*.js"`';
  ...
  c.on('exit', function(code) {
    ...
    console.log('You can now run doctests with `npm run jsdoctest` or `npm test`');
  });
};
...
exports._spec = function spec(program) {
  for (var i = 0, len = program.args.length; i < len; i++) {
    ...
    console.log(
      beautify(mocha.contentsToMochaSpec(rootDir, filename, content), {
        indent_size: 2,
        wrap_line_length: 80
      })
    );
