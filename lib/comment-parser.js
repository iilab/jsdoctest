...
exports.parseComments = function commentParser$parseComments(input) {
  ...
  for (var i = 0, len = input.length; i < len; i += 1) {
    ...
    // console.log('insideSinglelineComment',insideSinglelineComment)
    // console.log('insideMultiSinglelineComment',insideMultiSinglelineComment)
    // console.log('insideMultiSingleMaybe',insideMultiSingleMaybe)
    if (insideMultilineComment) {
      ...
      if (input[i] === '\n' && input[i+1] !== '/' && input[i+1] !== ' ') {
        ...
        // console.log('END OF SINGLE - parsed newline and nextline not a comment this was a singliline comment and we flush')
        flush();
