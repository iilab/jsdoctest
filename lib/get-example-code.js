...
function getExampleCode(comment) {
  ...
  if (isAsync) {
    ...
    return (
      ...
      'returnFunction.should.not.Throw();' +
      'var close = function () {' +
      ' listeners.forEach((l) => process.listeners("uncaughtException", l));' +
      ' recordedError && recordedError.should.be.an("error");' +
      ' recordedError && recordedError.should.have.property("message").and.have.string(' +
      expectedResult +
      ' );' +
      ' if (recordedError) return done();' +
      '}'
    );
  } else if (isPromise) {
    const promiseCode =
